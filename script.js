...
function renderMatchDetail(match){ const events=getMatchEvents(match.MatchID||match.ID); const first=events.filter(e=>getHalfNumber(e.Half)===1); const second=events.filter(e=>getHalfNumber(e.Half)===2); const youtube=match.YouTubeURL||match.YoutubeURL||match.HighlightsURL||''; const penalty=getPenaltyWinnerText(match); return `<section class="match-hero"><div class="match-date-main">${escapeHTML(formatFullDateTime(match.Date,match.Time))}</div><div class="match-main-teams"><div class="match-main-team"><div class="match-main-logo">${match.HomeLogo?`<img src="${escapeAttr(match.HomeLogo)}" alt="">`:''}</div><strong>${escapeHTML(match.HomeTeam)}</strong></div><div class="match-main-score"><div>${renderScoreText(match)}</div>${penalty?`<span>${escapeHTML(penalty)}</span>`:''}</div><div class="match-main-team"><div class="match-main-logo">${match.AwayLogo?`<img src="${escapeAttr(match.AwayLogo)}" alt="">`:''}</div><strong>${escapeHTML(match.AwayTeam)}</strong></div></div></section><section class="venue-row"><span>🏟️ Venue:</span><strong>${escapeHTML(match.Venue||match.Stadium||'Venue unavailable')}</strong></section><section class="event-section">${renderHalfEvents('1ST HALF',first,match)}${renderHalfEvents('2ND HALF',second,match)}</section>${renderHighlights(youtube)}`; }
...
function getEventLabel(event,liveHome,liveAway){ const type=String(event.Event||'').toLowerCase().trim(), detail=String(event.Detail||'').trim(), player=String(event.Player||'').trim(); const detailText=detail?` (${escapeHTML(detail)})`:''; if(type==='goal') return `<span class="goal-pill">⚽ ${liveHome} - ${liveAway}</span><strong>${escapeHTML(player)}${detailText}</strong>`; if(type==='red card') return `<span>🟥</span><strong>${escapeHTML(player)}${detailText}</strong>`; if(type==='penalty missed'||type==='missed penalty') return `<span>❌</span><strong>${escapeHTML(player)} (Penalty missed)</strong>`; return `<span>•</span><strong>${escapeHTML(player)}${detailText}</strong>`; }
...
window.CALCIUM_SCRIPT_VERSION='7000-clean-file-release';
